...
<div class="container-fluid px-4">
    <div class="pes-page-header"><h1 class="h3 mb-0">Gestión de Proveedores</h1>
    <p class="mb-2">Aquí puede agregar, editar y desactivar proveedores.</p></div>
                        ...
                        <table class="table table-bordered pes-table" id="dataTable" width="100%" cellspacing="0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">Embarcaciones</th>
                            ...
                            <tbody>
                                <?php
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        echo "<tr>";
                                            echo "<td class='text-center'>" . $row['nombre_proveedor'] . "</td>";
                                            echo "<td class='text-center'>" . $row['rut_proveedor'] . "</td>";
                                            echo "<td class='text-center'>" . $row['telefono_proveedor'] . "</td>";
                                            echo "<td class='text-center'>" . $row['domicilio_proveedor'] . "</td>";
                                            echo "<td class='text-center'>" . $row['embarcaciones'] . "</td>";
                                            echo "<td class='text-center'>" . ($row['estado'] == 1 ? 'Activo' : 'Inactivo') . "</td>";
                                            echo "<td class='text-center'>";
                                            echo "<a href='#' class='btn btn-warning btn-sm text-center btnEditar' data-id='" . $row['id_proveedor'] . "'>Editar</a> ";
                                            if($row['estado'] == 1){
                                                echo "<a href='#' class='btn btn-danger btn-sm text-center btnEliminar' data-id='" . $row['id_proveedor'] . "'>Desactivar</a> ";
                                            }else{
                                                echo "<a href='#' class='btn btn-success btn-sm text-center btnActivar' data-id='" . $row['id_proveedor'] . "'>Activar</a> ";
                                            }
                                            if($_SESSION['id_rol'] == 1){
                                                echo "<a href='#' class='btn btn-danger btn-sm text-center btnEliminar2' data-id='" . $row['id_proveedor'] . "'><i class='fa fa-times' aria-hidden='true'></i></a>";
                                            }
                                            echo "</td>";
                                        echo "</tr>";
                                    }                                    
                            ...
                            <div class="modal-content">
                                <form id="formAgregarUsuario" method="POST">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="modalAgregarUsuarioLabel">Agregar Proveedor</h5>
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control select2" id="embarcacion" name="embarcacion[]" multiple required>
                                                <?php
                            ...
                            <div class="modal-content">
                                <form id="formEditar" method="POST">
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="form-group">
                                            <label for="domicilio">Embarcacion</label>
                                            <select class="form-control select2" id="embarcacion2" name="embarcacion[]" multiple required>
                                                <?php
                                                    $sql = "SELECT id_embarcacion2, nombre_embarcacion2 FROM embarcaciones2";
                                                    $resultado = $connect->query($sql);
                                                    while ($fila = $resultado->fetch_assoc()) {
...
<script>
    $(document).ready(function() {
        // Inicializa Select2
        $('.select2').select2({
            placeholder: "Seleccione una o más embarcaciones",
            allowClear: true, // Permite borrar las selecciones
            width: '100%' // Asegura que el select use todo el ancho disponible
        });
    });
</script>
...
<script>
    $(document).ready(function () {
        // Limpia los campos del formulario al abrir el modal de Agregar Proveedor
        $('#modalAgregarUsuario').on('show.bs.modal', function () {
            $('#formAgregarUsuario')[0].reset(); // Resetea todos los campos del formulario
        });

        // Limpia los campos del formulario al abrir el modal de Editar Proveedor
...
<script>
    $(document).ready(function() {
        // Enviar el formulario de agregar proveedor por AJAX
...
<script>
    $(document).on('click', '.btnEliminar', function(e) {
        ...
        Swal.fire({
            title: '¿Está seguro?',
            text: "El proveedor no estará disponible para crear movimientos.",
            icon: 'warning',
...
<script>
    $(document).on('click', '.btnEliminar2', function(e) {
        ...
        Swal.fire({
            title: '¿Está seguro?',
            text: "El proveedor no se mostrará más en el sistema.",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
...
<script>
    $(document).on('click', '.btnActivar', function(e) {
        ...
        Swal.fire({
            title: '¿Está seguro?',
            text: "El proveedor estará disponible para crear movimientos.",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
    ...
    $(document).on('click', '.btnEditar', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#id_usuario').val(data.id_proveedor);
                $('#nombre2').val(data.nombre_proveedor);
                $('#rut2').val(data.rut_proveedor);
                $('#telefono2').val(data.telefono_proveedor);
                $('#domicilio2').val(data.domicilio_proveedor);
                var embarcaciones = data.embarcacion.split(',');
                $('#embarcacion2').val(embarcaciones);
                $('#embarcacion2').trigger('change');
