<?php
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id_usuario'])) {
        ...
        $query = "UPDATE trabajadores SET estado = 2 WHERE id_trabajador = $id_usuario";

        if (mysqli_query($connect, $query)) {
            echo "Trabajador desactivado correctamente";
        } else {
            echo "Error al desactivar trabajador: " . mysqli_error($connect);
        }
