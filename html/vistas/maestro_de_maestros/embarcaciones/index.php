...
<div class="container-fluid px-4">
    <div class="pes-page-header"><h1 class="h3 mb-0">Gestión de Embarcaciones</h1>
    <p class="mb-2">Aquí puede agregar, editar y desactivar embarcaciones.</p></div>

    <div class="row justify-content-center">
        <div class="col-lg-12">
            <div class="card shadow pes-card">
                <div class="card-header py-3 pes-card-header">
                    <h6 class="m-0 font-weight-bold text-primary">Listado de Embarcaciones</h6>
                </div>
                <div class="card-header py-3 pes-card-header">
                            ...
                            <tbody>
                                <?php
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        echo "<tr>";
                                            echo "<td class='text-center'>" . $row['nombre_embarcacion2'] . "</td>";
                                            echo "<td class='text-center'>" . ($row['estado'] == 1 ? 'Activo' : 'Inactivo') . "</td>";
                                            echo "<td class='text-center'>";
                                            echo "<a href='#' class='btn btn-warning btn-sm text-center btnEditar' data-id='" . $row['id_embarcacion2'] . "'>Editar</a> ";
                                            if($row['estado'] == 1){
                                                echo "<a href='#' class='btn btn-danger btn-sm text-center btnEliminar' data-id='" . $row['id_embarcacion2'] . "'>Desactivar</a> ";
                                            }else{
                                                echo "<a href='#' class='btn btn-success btn-sm text-center btnActivar' data-id='" . $row['id_embarcacion2'] . "'>Activar</a> ";
                                            }
                                            if($_SESSION['id_rol'] == 1){
                                                echo "<a href='#' class='btn btn-danger btn-sm text-center btnEliminar2' data-id='" . $row['id_embarcacion2'] . "'><i class='fa fa-times' aria-hidden='true'></i></a>";
                                            }
                                            echo "</td>";
                                        echo "</tr>";
                                    }                                    
                            ...
                            <div class="modal-content">
                                <form id="formAgregarUsuario" method="POST">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="modalAgregarUsuarioLabel">Agregar Embarcación</h5>
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <button type="submit" class="btn btn-primary">Guardar Embarcación</button>
                                    </div>
                                </form>
                            </div>
                        </div>
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <form id="formEditar" method="POST">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="modalEditarLabel">Editar Embarcación</h5>
                                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
        ...
        $('#formAgregarUsuario').on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    Swal.fire(
                        'Embarcación agregada',
                        'El embarcacion ha sido agregada correctamente.',
                        'success'
                    ).then(() => {
                ...
                error: function() {
                    Swal.fire(
                        'Error',
                        'No se pudo agregar la embarcacion.',
                        'error'
                    );
                }
            });
...
<script>
    $(document).on('click', '.btnEliminar', function(e) {
        ...
        Swal.fire({
            title: '¿Está seguro?',
            text: "La embarcacion no estará disponible para crear movimientos.",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        Swal.fire(
                            'Error',
                            'No se pudo desactivar la embarcacion.',
                            'error'
                        );
                    }
                });
...
<script>
    $(document).on('click', '.btnEliminar2', function(e) {
        ...
        Swal.fire({
            title: '¿Está seguro?',
            text: "La embarcacion no se mostrará más en el sistema.",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        Swal.fire(
                            'Error',
                            'No se pudo eliminar la embarcacion.',
                            'error'
                        );
                    }
                });
...
<script>
    $(document).on('click', '.btnActivar', function(e) {
        ...
        Swal.fire({
            title: '¿Está seguro?',
            text: "LA embarcacion estará disponible para crear movimientos.",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        Swal.fire(
                            'Error',
                            'No se pudo activar la embarcacion.',
                            'error'
                        );
                    }
                });
    ...
    $(document).on('click', '.btnEditar', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                $('#id_usuario').val(data.id_embarcacion2);
                $('#nombre2').val(data.nombre_embarcacion2);
    ...
    $(document).on('submit', '#formEditar', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                Swal.fire(
                    '¡Embarcación editada!',
                    'Los datos de la embarcacion han sido actualizados.',
                    'success'
                ).then(() => {
