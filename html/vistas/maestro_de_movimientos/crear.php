...
<div class="container-fluid px-4">
    ...
    <div class="pes-page-header"><h1 class="h3 mb-0">Movimientos</h1>

    <p class="mb-2">En esta sección podrá registrar <b><?php echo $movimiento; ?></b>.</p></div>
        ...
        <div class="col-lg-10">
            ...
            <div class="card shadow pes-card" id="imprimir">
                ...
                <div class="card-header py-3 pes-card-header">
                    <h6 class="m-0 font-weight-bold text-primary">Registro de movimiento</h6>
                </div>
                ...
                <div class="card-body">
                    ...
                    <form method="POST" id="egreso_create">
                        ...
                        <div class="d-flex justify-content-center align-items-center">
                            <div class="row">
                                <div class="form-group col-lg-12">
                                    ...
                                    <select class="form-control" name="tipo_movimiento" id="tipo_movimiento" required disabled>
                                        <option value="" <?= (!isset($tipo) || $tipo == '') ? 'selected' : '' ?>>Seleccionar</option>
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-lg-3">
                                ...
                                <select class="form-control" name="proveedor" id="proveedor" required>
                                    ...
                                    <?php 
                                    require '../../includes/db_connect.php';
                                    $sql = "SELECT * FROM proveedores WHERE estado = 1 AND eliminado = 0 ORDER BY nombre_proveedor ASC";
                                    $resultado = mysqli_query($connect, $sql) or die("Error: " . mysqli_error($connect));
                                    while($row = mysqli_fetch_assoc($resultado)){
                        ...
                        <button type="button" id="agregar-fila" class="btn btn-primary" style="display: none;">
                            <i class="fa fa-plus"></i> Agregar fila
                        </button>

                        <button type="button" id="eliminar-fila" class="btn btn-danger" style="display: none;">
                            <i class="fa fa-minus"></i> Eliminar fila
                        </button>

                        <div class="row" style="display: none;" id="div_total_final">
                            <div class="col-md-10"></div>
                            <div class="col-md-2">
                                <label for="total_final" class="text-gray-600 font-weight-bold">Total final</label>
                                <input type="text" value="$" id="total_final" class="form-control" readonly />
                            </div>
                        </div>

                        <div class="text-center mt-4">
                            <button type="button" class="btn btn-primary" id="guardar">Registrar movimiento</button>
                        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#agregar-fila', function() {
            ...
            const index = $('#lineas-dinamicas .form-group').length;

            $('#lineas-dinamicas').append(`
                <div class="form-group">
                    <div class="row">
                        <div class="col-md-2">
                            <label for="cheques_cantidad_${index}" class="text-gray-600">Cantidad de cheques</label>
                            <input type="number" min="0" value="0" class="form-control cheques-cantidad" id="cheques_cantidad_${index}" name="cheques_cantidad[]" data-index="${index}" required />
                        </div>
                        <div class="col-md-2">
                            <label for="anticipo_${index}" class="text-gray-600">Anticipo</label>
                            <input type="text" min="0" value="$" class="form-control" oninput="formatearMonto(event); actualizarTotalesFila(${index})" id="anticipo_${index}" name="anticipo[]" required />
                        </div>
                        <div class="col-md-4">
                            <label for="anticipo_${index}" class="text-gray-600">Detalle</label>
                            <input type="text" class="form-control" id="detalle_anticipo_${index}" name="detalle_anticipo[]" required />
                        </div>
                        <div class="col-md-2">
                            <label for="clasificacion_${index}" class="text-gray-600">Clasificación</label>
                            <select class="form-control" id="clasificacion_${index}" name="clasificacion[]" required></select>
                        </div>
                        <div class="col-md-2">
                            <label for="total_${index}" class="text-gray-600 font-weight-bold">Total</label>
                            <input type="text" value="$" class="form-control" oninput="formatearMonto(event)" id="total_${index}" name="total[]" readonly />
                        </div>
                    </div>
                    <div id="cheques_filas_${index}">
                        <!-- Filas dinámicas para Fecha y Total cheque aparecerán aquí -->
                    </div>
            ...
            if(trabajador != 0){
                $.ajax({
                    url: 'obtener_clasificaciones_trabajador.php',
                    method: 'GET',
                    success: function(opciones) {
                        $(`#clasificacion_${index}`).html(opciones);
                    },
                    error: function() {
                        alert('Error al cargar las clasificaciones.');
                    }
                });
            }else{
                $.ajax({
                    url: 'obtener_clasificaciones.php',
                    method: 'GET',
                    success: function(opciones) {
                        $(`#clasificacion_${index}`).html(opciones);
                    },
        ...
        $(document).on('click', '#eliminar-fila', function() {
            const filas = $('#lineas-dinamicas .form-group');
            if (filas.length > 1) {
                filas.last().remove();
            } else {
                Swal.fire('Error', 'El mínimo de filas es 1.', 'error');
            }
            actualizarTotalFinal();
        });
        ...
        <?php if ($tipo == 3){ ?>
            ...
            $('#embarcacion').on('change', function() {
            
                const embarcacionSeleccionado = $(this).val();

                if (embarcacionSeleccionado === "") {
                    $('#lineas-dinamicas').empty();
                    $('#agregar-fila').hide();
                    $('#eliminar-fila').hide();
                } else {

                    const selectedEmbarcacion = $('#embarcacion option:selected').map(function() {
                        return {
                            id: $(this).val(),
                    ...
                    selectedEmbarcacion.forEach(function(proveedor, index) {
                        $('#lineas-dinamicas').append(`
                            <div class="form-group">
                                <hr class="my-4 border-primary">
                                <div class="row">
        ...
        $('#proveedor').on('change', function() {
            ...
            if (proveedorSeleccionado === "") {
                ...
                const selectedProveedores = $('#proveedor option:selected').map(function() {
                    return {
                        id: $(this).val(),
                        nombre: $(this).data('nombre')
                    };
                ...
                selectedProveedores.forEach(function(proveedor, index) {
                    $('#lineas-dinamicas').append(`
                        <div class="form-group">
                            <hr class="my-4 border-primary">
                            <div class="row">
        ...
        $('#trabajador').on('change', function() {
            ...
            if (trabajadorSeleccionado === "") {
                ...
                const selectedTrabajador = $('#trabajador option:selected').map(function() {
                    return {
                        id: $(this).val(),
                        nombre: $(this).data('nombre')
                    };
                ...
                selectedTrabajador.forEach(function(trabajador, index) {
                    $('#lineas-dinamicas').append(`
                        <div class="form-group">
                            <hr class="my-4 border-primary">
                            <div class="row">
                        ...
                        try {
                            ...
                            if (data.status === 'success') {
                                Swal.fire({
                                    ...
                                    downloadLink.href = 'generar_pdf.php?id_movimiento=' + data.id_movimiento;
